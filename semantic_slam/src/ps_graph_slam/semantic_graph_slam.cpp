  ...
  if (empty_keyframe_queue()) {
    ...
    for (int i = 0; i < new_keyframes_.size(); ++i) {
      if (!new_keyframes_[i]->obj_info.empty()) {
        ...
        std::vector<landmark> current_landmarks_vec =
            this->semantic_oakd_data_ass(new_keyframes_[i]);
...
std::vector<landmark> semantic_graph_slam::semantic_oakd_data_ass(const ps_graph_slam::KeyFrame::Ptr curr_keyframe)
{
  // keyframe -> camara = robot con respecto mundo
    ...
    sem_tool_obj.transformNormalsToWorld(current_robot_pose,
                                         transformation_mat,
                                         cam_angle_);
    //Transformar de camara al mundo para aplicarlo al mundo
    ...
    for (int i = 0; i < object_info.size(); ++i){
        ...
        object.plane_type = "vertical";

        object.pose(0) = cloud_data.points[i].x;
        object.pose(1) = cloud_data.points[i].y;
        object.pose(2) = cloud_data.points[i].z;

        object.normal_orientation(0) = 0;
        object.normal_orientation(1) = 0;
        object.normal_orientation(2) = 1;
        

        Eigen::Vector4f pose_tmp;
        // pose del objeto respecto al robot en nuestro caso (camara en hdriay)
        pose_tmp(0) = object.pose(0);
        pose_tmp(1) = object.pose(1);
        pose_tmp(2) = object.pose(2);
        pose_tmp(3) = 1;

        // Hacer con tfs, revisar
        Eigen::Vector4f point_world_frame = transformation_mat * pose_tmp; // pose del objeto respecto mundo
