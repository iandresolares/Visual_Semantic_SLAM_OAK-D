...
void semantic_graph_slam_ros::init(ros::NodeHandle n) {
  ...
  odom_offset.pose.pose.orientation.w = 99;
  ...
  ros::param::param<bool>("~use_gt_fake_odom_", use_gt_fake_odom_, false);
...
void semantic_graph_slam_ros::open(ros::NodeHandle n) {
  ...
  oakd_sub_ = n.subscribe(
      "/detections",1, &semantic_graph_slam_ros::detectedObjectOakdCallback, this);

  oak_odom_sub_ = n.subscribe(
      "/camera/odom/sample", 1, &semantic_graph_slam_ros::oakVIOCallback, this);
...
void semantic_graph_slam_ros::oakVIOCallback(const nav_msgs::OdometryConstPtr &odom_msg){
  if (!use_gt_fake_odom_){
    if (odom_offset.pose.pose.orientation.w != 99){
      const ros::Time& stamp = odom_msg->header.stamp;
      Eigen::Isometry3d odom;
      Eigen::MatrixXf odom_cov;
      static int cont = 0;
      nav_msgs::Odometry::Ptr odom_ptr(new nav_msgs::Odometry);
      *odom_ptr = *odom_msg;
      // odom_ptr->pose.pose.position.x = odom_ptr->pose.pose.position.x - 1.42579;
      // odom_ptr->pose.pose.position.y = odom_ptr->pose.pose.position.y + 1.37078;
      // odom_ptr->pose.pose.position.z = odom_ptr->pose.pose.position.z + 1.40033;

      odom_ptr->pose.pose.position.x = odom_ptr->pose.pose.position.x + odom_offset.pose.pose.position.x;
      odom_ptr->pose.pose.position.y = odom_ptr->pose.pose.position.y + odom_offset.pose.pose.position.y;
      odom_ptr->pose.pose.position.z = odom_ptr->pose.pose.position.z + odom_offset.pose.pose.position.z;

      // tf2::Quaternion q_orig, q_rot, q_new;
      // tf2::convert(odom_ptr->pose.pose.orientation , q_orig);
      // q_rot.setRPY(-M_PI/2,0,-M_PI/2);
      // q_new = q_orig*q_rot;
      // q_new.normalize();
      // tf2::convert(q_new, odom_ptr->pose.pose.orientation); 

      // odom_ptr->pose.pose.position.z+= (float)cont/10000;
      // odom_ptr->pose.pose.position.x+= (float)cont/10000;

      odom = ps_graph_slam::odom2isometry(odom_ptr);
      odom_cov = ps_graph_slam::arrayToMatrix(odom_ptr);
...
void semantic_graph_slam_ros::detectedObjectOakdCallback(const semantic_SLAM::BoundingBoxes &msg){
  ...
  for (int i = 0; i < msg.bounding_boxes.size(); ++i) {
    ...
    object_info[i].width =
        abs(msg.bounding_boxes[i].xmax - msg.bounding_boxes[i].xmin);
    object_info[i].prob = msg.bounding_boxes[i].probability;
    // object_info[i].depth = msg.bounding_boxes[i].depth; //CAMBIAR en funcion del rosbag

    point.z = msg.bounding_boxes[i].depth;
    point.y = msg.bounding_boxes[i].y_centroid; 
    point.x = -msg.bounding_boxes[i].x_centroid;
...
void semantic_graph_slam_ros::publishLandmarks() {
  ...
  for (int i = 0; i < l_vec.size(); ++i) {
    ...
    if (l_vec[i].type == "chair") {
      ...
      marker.color.g = 1.0;
      marker.color.b = 0.0;
    } else if (l_vec[i].type == "diningtable") { // A AMARILLO
      marker.color.a = 1.0;  // Don't forget to set the alpha!
      marker.color.r = 1.0;
      marker.color.g = 1.0;
      marker.color.b = 0.0;
    } else if (l_vec[i].type == "bottle") { // A AZUL
      marker.color.a = 1.0;  // Don't forget to set the alpha!
      marker.color.r = 0.0;
      marker.color.g = 0.0;
      marker.color.b = 1.0;
...
void semantic_graph_slam_ros::optitrackPoseCallback(
  ...
  // Orientate like the base_link for getting real rotatio error metrics
  geometry_msgs::PoseStamped temp_pose;
  temp_pose.pose = msg.pose.pose;
  tf2::Quaternion q_orig, q_rot, q_new;
  tf2::convert(temp_pose.pose.orientation , q_orig);
  q_rot.setRPY(0,M_PI/2,M_PI);
  q_new = q_orig*q_rot;
  q_new.normalize();
  tf2::convert(q_new, temp_pose.pose.orientation); 
  ...
  optitrack_pose.pose = temp_pose.pose;
...
void semantic_graph_slam_ros::computeATE() {
  if (compute_txt_for_ate_) {
    ...
    robot_data.open("/home/andres/pruebas_ss/trayectory_analysis/stamped_traj_estimate.txt",
    ...
    vio_data.open("/home/andres/pruebas_ss/trayectory_analysis/vio_pose.txt",
                  std::ios::out | std::ios::ate | std::ios::app);
    ...
    gt_data.open("/home/andres/pruebas_ss/trayectory_analysis/stamped_groundtruth.txt",
                 std::ios::out | std::ios::ate | std::ios::app);
